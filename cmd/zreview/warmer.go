...
// place. Overridable via ZREVIEW_SESSION_DIR (same root as sessions).
func warmerLogDir() (string, error) {
	if dir := os.Getenv("ZREVIEW_SESSION_DIR"); dir != "" {
	...
	return filepath.Join(home, ".zreview"), nil
}
...
// spawnIndexWarmer launches `zreview index --paths <csv>` as a detached
// subprocess and returns immediately. The child inherits the parent's env
// (including ZREVIEW_DB_URL + provider creds) but writes its own stdio to a
