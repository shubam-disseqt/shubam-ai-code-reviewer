...
import (
	...
	dashboard "github.com/shubam-disseqt/z-code-reviewer/internal/metrics_dashboard"
)

// newMetricsCmd wires `zreview metrics` — a read-only HTML report over the
// session JSONL logs in ~/.zreview/sessions. Purely local; no LLM calls.
func newMetricsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "metrics",
		Short: "Generate an HTML dashboard of past zreview runs",
...
renders in any browser without a build step.`,
	...
	cmd.Flags().StringVar(&sessionsDir, "sessions-dir", "", "session log directory (default ~/.zreview/sessions)")
	cmd.Flags().StringVar(&output, "output", "zreview-metrics.html", "output HTML path")
	return cmd
}

// resolveSessionsDir expands "" to ~/.zreview/sessions. Returns an error if
