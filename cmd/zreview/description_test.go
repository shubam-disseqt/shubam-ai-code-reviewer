...
func TestUpdateDescription_AppendsBlockWhenMissing(t *testing.T) {
	...
	err := UpdateDescription(context.Background(), f, "o", "r", 7,
		...
		model.Labels{},
		map[scoring.Severity]int{scoring.SeverityHigh: 1})
	if err != nil {
	...
	if !strings.Contains(f.updated, zreviewBlockBegin) || !strings.Contains(f.updated, zreviewBlockEnd) {
...
func TestUpdateDescription_ReplacesExistingBlock(t *testing.T) {
	...
	if strings.Count(f.updated, zreviewBlockBegin) != 1 || strings.Count(f.updated, zreviewBlockEnd) != 1 {
...
func TestUpdateDescription_NoChangeSkipsUpdate(t *testing.T) {
	...
	block := renderZreviewBlock(model.Summary{Walkthrough: "same"}, model.Labels{}, map[scoring.Severity]int{})
...
func TestUpdateDescription_EmptyBodyGetsBlock(t *testing.T) {
	...
	if !strings.HasPrefix(f.updated, zreviewBlockBegin) {
...
func TestUpdateDescription_GetErrorPropagates(t *testing.T) {
	...
	err := UpdateDescription(context.Background(), f, "o", "r", 7,
		model.Summary{}, model.Labels{}, nil)
	if err == nil {
		t.Fatalf("want error")
	}
...
func TestUpdateDescription_UpdateErrorPropagates(t *testing.T) {
	...
	err := UpdateDescription(context.Background(), f, "o", "r", 7,
		model.Summary{Walkthrough: "hi"}, model.Labels{}, nil)
	if err == nil {
		t.Fatalf("want error")
	}
...
func TestUpdateDescription_MissingIdentity(t *testing.T) {
	f := &fakePRClient{}
	err := UpdateDescription(context.Background(), f, "", "r", 7, model.Summary{}, model.Labels{}, nil)
	if err == nil {
		t.Errorf("empty owner should error")
	}
	err = UpdateDescription(context.Background(), f, "o", "r", 0, model.Summary{}, model.Labels{}, nil)
	if err == nil {
		t.Errorf("zero pr should error")
	}
	err = UpdateDescription(context.Background(), nil, "o", "r", 7, model.Summary{}, model.Labels{}, nil)
	if err == nil {
		t.Errorf("nil client should error")
	}
}

func TestRenderZreviewBlock_ChangeGroups(t *testing.T) {
	block := renderZreviewBlock(
		model.Summary{
