...
import (
	...
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/sarif"
...
func emitGithub(ctx context.Context, cfg emitConfig) error {
	...
	// Delete stale sacr-authored comments before posting the new batch.
	...
	// last review run.
	//
	// Legacy: comments posted before sacr learned to embed a
	...
	//
	// Non-sacr comments (human reviewers, other bots) have neither the
	...
	// every existing sacr-authored comment (any fp) — the fresh batch
	...
	if existing, err := cfg.GHClient.ListReviewComments(ctx, cfg.Owner, cfg.Repo, cfg.PRNumber); err == nil {
		for _, e := range existing {
			...
			isSacr := fp != "" || looksLikeLegacySacrComment(e.Body)
			if !isSacr {
	...
	for i := 0; i < len(batch); i += reviewBatchSize {
		...
		chunk := batch[i:end]
		summary := fmt.Sprintf("sacr review (%d–%d of %d)", i+1, end, len(batch))
	...
	if scannerCount > 0 && os.Getenv("SACR_UPLOAD_SARIF") == "1" {
	...
	// it, not just in the standalone `sacr overlap` command's stdout.
...
// sacrFPMarkerPrefix is the HTML-comment marker prefix that sacr
...
// sacr-authored comments without a bot-user lookup.
const sacrFPMarkerPrefix = "<!-- sacr:fp:"
...
// looksLikeLegacySacrComment matches the prefix sacr's formatter
// emits ("**[<severity> / <category>]** ") without a fingerprint marker.
// Used to clean up pre-marker comments from earlier releases.
func looksLikeLegacySacrComment(body string) bool {
	if strings.Contains(body, sacrFPMarkerPrefix) {
		return false
	}
...
// extractSacrFingerprint pulls the fingerprint hex from an existing
// comment body, or "" if the comment isn't sacr-authored.
func extractSacrFingerprint(body string) string {
	i := strings.Index(body, sacrFPMarkerPrefix)
	if i < 0 {
		return ""
	}
	rest := body[i+len(sacrFPMarkerPrefix):]
