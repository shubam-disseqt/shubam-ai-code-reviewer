...
func runRulesSync(ctx context.Context, cmd *cobra.Command) error {
	...
	if source == "" {
		return fmt.Errorf("no rules source (set SACR_ORG_RULES_REPO)")
	}
	dir := rulesCacheDir()
