...
import (
	...
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/conventions"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/depgraph"
	...
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/reviewctx"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/rules"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/scoring"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/selector"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/session"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/tool"
	"github.com/shubam-disseqt/shubam-ai-code-reviewer/internal/zconfig"
)

// reviewOpts is the parsed CLI surface for `sacr review`.
...
// note: hardcoded 200k window, upgrade path is per-model config.
...
func runReview(ctx context.Context, cmd *cobra.Command, opts *reviewOpts) error {
	...
	// SACR_LOG_FORMAT=json for CI/observability.
	...
	// User-facing toggles (.sacr/config.yaml). Malformed file logs and
...
func loadRules(ctx context.Context, kept []model.Diff) (string, error) {
	...
	cacheDir := filepath.Join(os.TempDir(), "sacr-rules-cache")
...
func newSession(resumeID string) (*session.Session, error) {
	dir := os.Getenv("SACR_SESSION_DIR")
	if dir == "" {
...
func maybeDetectOverlap(ctx context.Context, opts *reviewOpts, kept []model.Diff, llmc llm.LLMClient) []overlap.Finding {
	if os.Getenv("SACR_OVERLAP_ENABLED") == "0" {
		return nil
	}
