...
// shared by the review and scan pre-flight gates, their large-input filters,
// and computeActiveZoneSize so the threshold has a single definition.
// Non-positive input is not special-cased — each caller decides what that
// means.
...
// around structured outputs. Exposed so callers that parse LLM JSON output
...
func (r *Runner) runCompression(ctx context.Context, msgs []llm.Message, taskKey string) ([]llm.Message, error) {
	...
	// Record the request before it goes out: the session log then witnesses
	// the request even if the process dies mid-call.
	fs := r.deps.Session.GetOrCreateFileSession(taskKey)
	rec := fs.AppendTaskRecord(MemoryCompressionTask, compressionMsgs)

	ctx = llm.ContextWithSessionKey(ctx,
		llm.SessionTaskKey(r.deps.Session.SessionID(), string(MemoryCompressionTask), taskKey))
