...
// of the upstream fix: once an assistant turn's Native payload (thinking
// blocks, reasoning items, encrypted_content, reasoning_content) actually
// replays on the wire, a token budget computed only from ExtractText() would
...
// PromptTokenLimit replaced a literal `maxTokens*4/5` at four call sites, so
// the float form must agree with the integer form it replaced across the
// realistic max_tokens range. This is specific to tokenWarningThreshold being
// 0.80 — if the threshold ever changes, delete this test rather than "fixing"
// it.
