...
// Adapted from alibaba/open-code-review internal/llm/responses_client.go
...
func (c *OpenAIResponsesClient) buildResponsesParams(model string, req ChatRequest) responses.ResponseNewParams {
	...
	for _, msg := range req.Messages {
		...
		case "tool":
			// openai-go v3.61 dropped the (callID, output) signature; construct
			// the param directly so we can still set CallID.
			fco := responses.ResponseInputItemFunctionCallOutputParam{
				CallID: param.NewOpt(msg.ToolCallID),
			}
			fco.Output.OfString = param.NewOpt(content)
			input = append(input, responses.ResponseInputItemUnionParam{OfFunctionCallOutput: &fco})
		default:
			input = append(input, responses.ResponseInputItemParamOfMessage(content, responses.EasyInputMessageRoleUser))
		}
	}
