...
//
// Adapted from alibaba/open-code-review internal/tool/code_comment.go
...
// parsed after the deterministic repair (see args_repair.go). It reports a
...
// ParseComments extracts LlmComment entries from tool call arguments. Returns
// parsed comments and an error message (empty on success).
//
...
func parseCommentsInner(args map[string]any, defaultPath string) ([]model.LlmComment, *CommentRepair, string) {
	...
	for _, raw := range rawComments {
		...
		if category, ok := obj["category"].(string); ok {
			cm.Category = normalizeCategory(category)
		}
		if severity, ok := obj["severity"].(string); ok {
			cm.Severity = normalizeSeverity(severity)
		}
