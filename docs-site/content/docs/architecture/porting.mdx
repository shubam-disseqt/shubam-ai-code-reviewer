---
title: Porting Spec
description: Per-file attribution map for upstream ports from alibaba/open-code-review and miracodeai/mira.
---

Synthesized from the Phase 1 research pass. Five parallel agents
mapped the two upstream projects onto our target Go layout. This file
is the per-file plan for Phases 3–8.
...
Both are Apache-2.0. Attribution is set out in `NOTICE` and each
lifted file carries the upstream copyright header verbatim per §4 of
the license.

## Provenance verdict per source file
...
## Attribution model (per-file header)
...
## Key algorithms called out

Three algorithms deserve special care because they are the
load-bearing correctness stories:

### Line-snapping (OCR — `internal/diff/resolver.go`)
...
### JIT context (Mira — `jit_context.py`)
...
### Overlap prefilter (Mira — `core/overlap.py`)
...
head or base ref matches the current PR's — stacked PRs share files
by design and flagging them is noise.

Batched LLM verdict then classifies survivors as `merge_conflict`,
`duplicate_effort`, `both`, or `none`, with a confidence 0..1.
Confidence < 0.6 is dropped.
...
## Modification-required list (what we owe on lift)
...
6. **Rules layer greenfield** — Mira's per-repo `review_context` table + `learned_rules` table are collapsed into a single YAML-in-git format. Scope filtering (currently dead in Mira — see the Gotchas section below) is wired up here.
...
## Test surface to lift alongside
...
Distilled from the Phase 1 "Surprises" sections. Read this before
writing any code in the corresponding package.
