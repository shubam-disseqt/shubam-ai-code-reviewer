...
Every `runReview()` call executes these stages in order. Each one wraps
its error with a `stage:` prefix so a failure names the phase. The dashed
edges are best-effort — a failure logs a warning and returns an empty
...
**Fatal stages** (abort with wrapped error): diff, scoring, session, LLM,
prompts, tools, emit.

**Fault-tolerant stages** (log warning, continue with empty result):
scanners, cheap-tier agents, index, overlap, effort, depgraph.
...
Not yet all built. Each row lists the target package and its job.
...
| `internal/rules` | Git-cloned org rules repo → YAML load → glob-scoped filter → prompt injection. YAML-in-git rather than a DB. |
...
| `internal/docs` | Embedded static docs + local HTTP server for `sacr docs`. Hardened with a host-header allowlist and default security headers. |
...
|---|---|---|
| T1 | Command injection via crafted diff content | External process execution in `internal/gitcmd` is restricted to `git`, hardcoded subcommands, `--end-of-options`, no shell interpolation |
...
| T3 | Path traversal via LLM-suggested file paths | `internal/pathutil.WithinBase()` validates all file paths against the repository root, pre- and post-symlink resolution |
| T4 | DNS rebinding against local `sacr docs` server | Host-header allowlist in `internal/docs` rejects requests from non-loopback origins; wildcard binds require explicit `SACR_DOCS_ALLOWED_HOSTS` |
...
- **No cross-repo dependency graph.** Intentionally out of scope.
