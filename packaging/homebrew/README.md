# Homebrew tap for zreview

The `brew install shubam-disseqt/tap/zreview` command in the root README depends
...
2. Copy `Formula/zreview.rb` from this directory into the root of that repo
   under `Formula/zreview.rb`.
3. Commit and push to `main`. The tap is now live.
4. Add a repository secret to `shubam-disseqt/z-code-reviewer` called
...
```sh
brew install shubam-disseqt/tap/zreview
```
...
1. Builds `zreview` for macOS/Linux on arm64 and amd64.
2. Uploads `zreview_<version>_<os>_<arch>.tar.gz` archives to the GitHub
   release.
...
```sh
brew install --build-from-source ./Formula/zreview.rb
brew test zreview
brew audit --strict --online zreview
```
